...
except ImportError:  # Python 3.8 (Isaac Gym env)
...
def fla_cache_autotune(configs, key=None, prune_configs_by=None, reset_to_zero=None, restore_value=None,
    ...
    def decorator(fn):
        ...
        # Triton < 3.4 Autotuner has a smaller __init__ (no do_bench, etc.)
        allowed_params = set(inspect.signature(Autotuner.__init__).parameters)
        autotune_kwargs = {
            k: v
            for k, v in autotune_kwargs.items()
